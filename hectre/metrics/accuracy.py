...
def cdf_accuracy(path_to_pdf: str, picos_string: str, path_to_cdf: str):
    ...
    test_cdf_native: Optional[CDF] = extract_data(file_path=path_to_test_pdf.resolve(), picos_string=picos_string)
    test_cdf = test_cdf_native.to_df()
    if not test_cdf_native:
