...
class Book(BaseModel):
    title: str
    author: str = Field(min_length=1, pattern="[A-Za-z ,.'-]+$")
    price: confloat(gt=0)
    year: Optional[int]
...
def test_create_book_with_valid_schema_successfully(valid_schema: dict) -> None:
    """
    Test case to verify the successful creation of a book with valid schema.
    """
    book = Book(
...
def test_create_book_with_invalid_schema_raises_exception(invalid_schema: dict) -> None:
    """
    Test case to verify that creating a book with an invalid schema raises a ValidationError.
    """
